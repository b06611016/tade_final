...
class ResNext(nn.Module):
    ...
    def __init__(self, block, layers, num_experts, groups=1, width_per_group=64, dropout=None, num_classes=1000, use_norm=False, reduce_dimension=False, layer3_output_dim=None, layer4_output_dim=None, returns_feat=False, s=30):
        ...
        self.rot_avgpool = nn.AdaptiveAvgPool2d(output_size=(1,1))
    ...
    def _separate_part(self, x, ind, rot_x=None):
        x = (self.layer3s[ind])(x)
        x = (self.layer4s[ind])(x)

        if rot_x is not None:
            rot_x = (self.layer3s[ind])(rot_x)
            rot_x = self.rot_avgpool(rot_x)
            rot_x = rot_x.view(rot_x.size(0), -1)
            # print(rot_x.shape)
            self.rot_feat.append(rot_x)
    ...
    def forward(self, x, epoch=None, epoch_length=None, rot_x=None):
        with autocast():
            ...
            # rotated image
            if rot_x is not None:
                rot_x = self.conv1(rot_x)
                rot_x = self.bn1(rot_x)
                rot_x = self.relu(rot_x)
                rot_x = self.maxpool(rot_x)

                rot_x = self.layer1(rot_x)
                rot_x = self.layer2(rot_x)
